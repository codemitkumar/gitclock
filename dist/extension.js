...
var CLIENT_ID = "Ov23liL2EQWqE969CVmX";
var CLIENT_SECRET = "25a877889310d4278e26bfa47e25a3115f2ebb1e";
var AUTH_URL = "https://github.com/login/oauth/authorize";
var TOKEN_URL = "https://github.com/login/oauth/access_token";
var REDIRECT_URI = "http://localhost:5000/oauthCallback";
var GITHUB_API_URL = "https://api.github.com";
var REPO_NAME = "gitClock-LifeCycle";
...
async function handleRepoAndChangelog(accessToken, changedFiles) {
  try {
    ...
    const changesList = changedFiles.map(
      (file) => `| ${(/* @__PURE__ */ new Date()).toLocaleString()} | ${file.fileName} | ${file.additions} Additions & ${file.deletions} Deletions|`
    ).join("\n");
    if (changelogFile) {
...
This file logs the changes made on ${(/* @__PURE__ */ new Date()).toLocaleDateString()}.
...
async function activate(context) {
  const disposable = vscode.commands.registerCommand(
    ...
    async function() {
      try {
        const authUrl = `${AUTH_URL}?client_id=${CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&scope=repo%20user`;
          ...
          if (req.url.startsWith("/oauthCallback")) {
            ...
            try {
              ...
              if (accessToken2) {
                ...
                context.globalState.update("githubAccessToken", accessToken2);
                checkAndCreateRepo(accessToken2);
              } else {
  ...
  const runNowDisposable = vscode.commands.registerCommand(
    "gitclock.runNow",
    async function() {
      const accessToken2 = context.globalState.get("githubAccessToken");
      if (!accessToken2) {
        vscode.window.showErrorMessage(
      ...
      await runSyncOnce(accessToken2);
    }
  );
...
function getDiffStats(cwd, fileName) {
  return new Promise((resolve) => {
    exec(`git diff --numstat -- "${fileName}"`, { cwd }, (error, stdout) => {
      if (error || !stdout.trim()) {
        resolve({ additions: void 0, deletions: void 0 });
        return;
      }
